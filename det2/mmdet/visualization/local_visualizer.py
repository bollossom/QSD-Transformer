...
"""
/home/ligq/qxr/bsnn/Spike-Driven-Transformer-V2/detection2/mmdet/visualization/local_visualizer.py定义了DetLocalVisualizer的hook，数据类型为RGBA，如果想改回来就需要搜索所有mask_draw有关的方法里的alphas令其=self.alpha
2. 在~/qxr/bsnn/Spike-Driven-Transformer-V2/detection2目录下执行python setup.py build install重新注册钩子，就改回来了
"""
...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'masks' in instances:
            ...
            self.draw_polygons(polygons, edge_colors='w', alpha=0.)
            self.draw_binary_masks(masks, colors=colors, alphas=0.)
    ...
    def _draw_panoptic_seg(self, image: np.ndarray,
        ...
        for i, mask in enumerate(segms):
            ...
            polygons.extend(contours)
        self.draw_polygons(polygons, edge_colors='w', alpha=0.)
        self.draw_binary_masks(segms, colors=colors, alphas=0.)
